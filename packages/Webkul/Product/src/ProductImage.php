...
class ProductImage
{
    ...
    {
        // If driver is not local, use direct storage URLs.
        if (! $this->isDriverLocal()) {
            // Prefer relative storage URL when file exists in public disk
            if (Storage::exists($path)) {
                $relative = url('/storage/'.$path);

                return [
                    'small_image_url'    => $relative,
                    'medium_image_url'   => $relative,
                    'large_image_url'    => $relative,
                    'original_image_url' => $relative,
                ];
            }
        ...
        // If image processing extensions are not available, avoid cache routes
        // which rely on Intervention Image and return the stored image URL directly.
