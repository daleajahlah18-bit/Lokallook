...
class VatValidator
{
    ...
    protected static $pattern_expression = [
        ...
        'BE' => '(0\d{9}|\d{10})',
        'BG' => '\d{9,10}',
        'BQ' => '\d{9,15}', // Bonaire, Sint Eustatius and Saba
        'CY' => '\d{8}[A-Z]',
        'CZ' => '\d{8,10}',
        'DE' => '\d{9}',
        'DK' => '(\d{2} ?){3}\d{2}',
        ...
        'CA' => '[A-Z\d]{15}', // Canadian GST
        'MX' => '[A-Z]{3,4}\d{6}[A-Z0-9]{3}', // Mexican RFC
        'SG' => '\d{8}[A-Z]', // Singapore GST
        'HK' => '\d{13}', // Hong Kong ID
        'IN' => '\d{15}', // India GST
        'ZA' => '\d{10}', // South Africa VAT
        'NZ' => '\d{9}', // New Zealand GST
        'XX' => '[\d\w]{1,20}', // Generic fallback for unknown countries
    ];
    ...
    public function validate(string $vatNumber, ?string $formCountry = null): bool
    {
        ...
        // Use form country as fallback if country code not found in patterns
        if (! isset(self::$pattern_expression[$country])) {
            if ($formCountry && isset(self::$pattern_expression[$formCountry])) {
                $country = $formCountry;
                $number = $vatNumber;
            } else {
                // If still no match, use the generic fallback pattern
                $country = 'XX';
            }
        }
